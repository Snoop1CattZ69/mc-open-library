...
public class ValueParser {
	...
	private final Pattern singlePattern = Pattern.compile("\\s*(chance\\s*([0-9]*[.,]?[0-9]+)%\\s*)?from\\s*([-+]?([0-9]*[.,]?[0-9]+%?))\\s*to\\s*((([-+]?([0-9]*[.,]?[0-9]+%?))))\\s*((per\\s*level\\s*([-+]?([0-9]*[.,]?[0-9]+%?))))?\\s*");
	private final Pattern rangePattern  = Pattern.compile("\\s*(chance\\s*([0-9]*[.,]?[0-9]+)%\\s*)?from\\s*([-+]?([0-9]*[.,]?[0-9]+)(-([0-9]*[.,]?[0-9]+)))\\s*to\\s*([-+]?([0-9]*[.,]?[0-9]+)(-([0-9]*[.,]?[0-9]+)))\\s*(per\\s*level\\s*([-+]?([0-9]*[.,]?[0-9]+)(-([0-9]*[.,]?[0-9]+))))?\\s*");
	 ...
	 */
	public RandomVal getRandomValue(String from) {
	 ...
	 */
	public RandomSingle getSingleValue(String from) {
	 ...
	 */
	public RandomRange getRangeValue(String from) {
		...
		if (perLevelValue != null && firstValue.split("-").length < 1)
			throw new IllegalArgumentException("Incorrect FIRST RANGE argument: " + from);

		if (perLevelValue != null && secondValue.split("-").length < 1)
			throw new IllegalArgumentException("Incorrect SECOND RANGE argument: " + from);

		if (perLevelValue != null && perLevelValue.split("-").length < 1)
			throw new IllegalArgumentException("Incorrect PER LEVEL RANGE argument" + from);
